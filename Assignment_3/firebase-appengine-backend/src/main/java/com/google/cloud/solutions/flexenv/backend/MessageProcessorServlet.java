...
public class MessageProcessorServlet extends HttpServlet {
  ...
  private static final String BUCKET = "playchat-4f2c8.appspot.com";
...
// [END initializeEventLogger]
  ...
  public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    // File naming stuff
    int suffix = 1;
    String name = "playchat-log";
    String extension = ".txt";
    
    // GCS File stuff
     GcsFileOptions instance = GcsFileOptions.getDefaultInstance();
     GcsFilename gcsFileName = new GcsFilename(BUCKET, name+extension);
     // Construct a new file name if the file already exists
     while (gcsService.getMetadata(gcsFileName) != null){
            gcsFileName = new GcsFilename(BUCKET, name + "_" + suffix + extension);
            suffix++;
     }
    ...
    // Setting some properties for the response that will be sent back
    ...
    for (Iterator<LogEntry> iter = logs.iterator(); iter.hasNext();) {
      ...
      // Formatting the data to be sent back and written to the file.
      String data = new Date(entry.getTimeLong()).toString() + "(id=" + entry.getTag()
        + ")" +  " : " + entry.getLog();
      resp.getWriter().println(data);         // Writting to the response
      writer.print(data);                     // Writting to the file
    }
    writer.close();
    outputChannel.close();
  }
